...
#include "luaffifb/types.h"	// CData

static int check_nil_vs_null(const TValue *t1, const TValue *t2) {
...
int luaV_equalobj (lua_State *L, const TValue *t1, const TValue *t2) {
  ...
  if (ttypetag(t1) != ttypetag(t2)) {  /* not the same variant? */
    ...
    // L==NULL for rawequal() so let's hope it's non-null for normal __eq
    if (L && check_nil_vs_null(t1, t2)) return 1;
