...
class value_type_mismatch : public std::logic_error {
  public:
    explicit value_type_mismatch(const std::string &what_arg) : std::logic_error(what_arg) {}
    explicit value_type_mismatch(const char *what_arg) : std::logic_error(what_arg) {}
};
...
class value {
  ...
  private:
    ...
    value(const unit *cu, DW_AT name, DW_FORM form, type typ, section_offset offset);
...
class expr_error : public std::runtime_error {
  public:
    explicit expr_error(const std::string &what_arg) : std::runtime_error(what_arg) {}
...
class expr_context {
  public:
     ...
     */
    virtual taddr reg(unsigned regnum) { throw expr_error("DW_OP_breg* operations not supported"); }
...
class rangelist {
  public:
     ...
     * the range list).
     */
    rangelist(const std::shared_ptr<section> &sec, section_offset off, unsigned cu_addr_size, taddr cu_low_pc);
...
class rangelist::iterator {
  public:
    ...
    /** Equality operator */
    bool operator==(const iterator &o) const { return sec == o.sec && pos == o.pos; }
...
class line_table {
  public:
     ...
     */
    line_table(const std::shared_ptr<section> &sec, section_offset offset, unsigned cu_addr_size,
               const std::string &cu_comp_dir, const std::string &cu_name);
...
class line_table::iterator {
  public:
    ...
    /** Equality operator */
    bool operator==(const iterator &o) const { return o.pos == pos && o.table == table; }
...
class die_str_map {
  public:
     ...
     */
    die_str_map(const die &parent, DW_AT attr, const std::initializer_list<DW_TAG> &accept);
     ...
     */
    const die &operator[](const std::string &val) const { return (*this)[val.c_str()]; }
...
struct hash<dwarf::unit> {
    ...
    result_type operator()(argument_type a) const { return hash<decltype(a.m)>()(a.m); }
};
