...
void canon_hdr(Hdr<Elf64, byte_order::native> *out, const void *data, elfclass ei_class, elfdata ei_data) {
    switch (ei_class) {
        case elfclass::_32:
            switch (ei_data) {
                case elfdata::lsb:
...
elf::elf(const std::shared_ptr<loader> &l) : m(make_shared<impl>(l)) {
    ...
    if (core_hdr->ei_class != elfclass::_32 && core_hdr->ei_class != elfclass::_64) throw format_error("bad ELF class");
    ...
    if (m->hdr.shnum && m->hdr.shstrndx >= m->hdr.shnum) throw format_error("bad section name string table index");
...
const char *section::get_name(size_t *len_out) const {
    ...
    if (!m->name) m->name = m->f.get_section(m->f.get_hdr().shstrndx).as_strtab().get(m->hdr.name, &m->name_len);
...
strtab section::as_strtab() const {
    if (m->hdr.type != sht::strtab) throw section_type_mismatch("cannot use section as strtab");
...
symtab section::as_symtab() const {
    ...
    return symtab(m->f, data(), size(), m->f.get_section(get_hdr().link).as_strtab());
}
...
struct strtab::impl {
    impl(const elf &f, const char *data, const char *end) : f(f), data(data), end(end) {}
...
const char *strtab::get(Elf64::Off offset, size_t *len_out) const {
    ...
    if (start >= m->end) throw range_error("string offset " + std::to_string(offset) + " exceeds section size");
...
std::string strtab::get(Elf64::Off offset) const { return get(offset, nullptr); }
...
const char *sym::get_name(size_t *len_out) const { return strs.get(get_data().name, len_out); }
...
struct symtab::impl {
    impl(const elf &f, const char *data, const char *end, strtab strs) : f(f), data(data), end(end), strs(strs) {}
...
symtab::symtab(elf f, const void *data, size_t size, strtab strs)
    : m(make_shared<impl>(f, (const char *)data, (const char *)data + size, strs)) {}

symtab::iterator::iterator(const symtab &tab, const char *pos) : f(tab.m->f), strs(tab.m->strs), pos(pos) {
