...
section_offset die::get_section_offset() const { return cu->get_section_offset() + offset; }
...
die::iterator::iterator(const unit *cu, section_offset off) : d(cu) { d.read(off); }
...
bool die::operator==(const die &o) const { return cu == o.cu && offset == o.offset; }
...
size_t std::hash<dwarf::die>::operator()(const dwarf::die &a) const {
    return hash<decltype(a.cu)>()(a.cu) ^ hash<decltype(a.get_unit_offset())>()(a.get_unit_offset());
}
