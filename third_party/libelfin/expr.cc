...
#define CHECKN(n)                             \
    do {                                      \
        ...
        switch (op) {
            ...
            case DW_OP::deref_size:
                tmp1.u = cur.fixed<uint8_t>();
                if (tmp1.u > subsec->addr_size) throw expr_error("DW_OP_deref_size operand exceeds address size");
            deref_common:
                CHECK();
            ...
            case DW_OP::xderef_size:
                tmp1.u = cur.fixed<uint8_t>();
                if (tmp1.u > subsec->addr_size) throw expr_error("DW_OP_xderef_size operand exceeds address size");
            xderef_common:
                CHECKN(2);
                tmp2.u = stack.back();
                stack.pop_back();
...
#undef SRELOP
            ...
            default:
                if ((((ubyte)op) >= ((ubyte)DW_LNE::lo_user)) && (((ubyte)op) <= ((ubyte)DW_LNE::hi_user))) {
...
#undef CHECKN
    ...
    if (result.location_type == expr_result::type::address) {
        ...
        if (stack.empty()) throw expr_error("final stack is empty; no result given");
        result.value = stack.back();
    }
