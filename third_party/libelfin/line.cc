...
line_table::line_table(const shared_ptr<section> &sec, section_offset offset, unsigned cu_addr_size,
                       const string &cu_comp_dir, const string &cu_name)
    : m(make_shared<impl>()) {
    ...
    if (version < 2 || version > 4) throw format_error("unknown line number table version " + std::to_string(version));
    ...
    if (m->line_range == 0) throw format_error("line_range cannot be 0 in line number table");
    m->opcode_base = cur.fixed<ubyte>();

    static_assert(sizeof(opcode_lengths) / sizeof(opcode_lengths[0]) == 13, "opcode_lengths table has wrong length");
    ...
    for (unsigned i = 1; i < m->opcode_base; i++) {
        ...
        if (length != opcode_lengths[i])
            ...
            throw format_error("expected " + std::to_string(opcode_lengths[i]) + " arguments for line number opcode " +
...
line_table::iterator line_table::find_address(taddr addr) const {
    ...
    iterator it = prev;
    for (++it; it != e; prev = it++) {
        if (prev->address <= addr && it->address > addr && !prev->end_sequence) return prev;
    }
    prev = e;
    return prev;
}
...
const line_table::file *line_table::get_file(unsigned index) const {
    if (index >= m->file_names.size()) {
        ...
        if (index >= m->file_names.size())
            throw out_of_range("file name index " + std::to_string(index) + " exceeds file table size of " +
...
bool line_table::impl::read_file_entry(cursor *cur, bool in_header) {
    ...
    if (file_name[0] == '/')
        ...
        file_names.emplace_back(include_directories[dir_index] + file_name, mtime, length);
    else
        throw format_error("file name directory index out of range: " + std::to_string(dir_index));

    return true;
}

line_table::file::file(string path, uint64_t mtime, uint64_t length) : path(path), mtime(mtime), length(length) {}
...
line_table::iterator::iterator(const line_table *table, section_offset pos) : table(table), pos(pos) {
    if (table) {
...
line_table::iterator &line_table::iterator::operator++() {
    ...
    if (output) {
        ...
        if (entry.file_index < table->m->file_names.size())
            ...
            throw format_error("bad file index " + std::to_string(entry.file_index) + " in line table");
    }
...
bool line_table::iterator::step(cursor *cur) {
    ...
    if (opcode >= m->opcode_base) {
        ...
        regs.line += line_inc;
        regs.address +=
            m->minimum_instruction_length * ((regs.op_index + op_advance) / m->maximum_operations_per_instruction);
        ...
        switch ((DW_LNS)opcode) {
            ...
            advance_pc:
                regs.address +=
                    m->minimum_instruction_length * ((regs.op_index + uarg) / m->maximum_operations_per_instruction);
            ...
            default:
                ...
                throw format_error("unknown line number opcode " + to_string((DW_LNS)opcode));
        }
        ...
        switch ((DW_LNE)opcode) {
            ...
            case DW_LNE::set_discriminator:
                ...
                break;
            default:
                if ((opcode >= ((ubyte)DW_LNE::lo_user)) && (opcode <= ((ubyte)DW_LNE::hi_user))) {
                    // XXX Vendor extensions
                    throw runtime_error("vendor line number opcode " + to_string((DW_LNE)opcode) + " not implemented");
                    // XXX Prior to DWARF4, any opcode number
                    // could be a vendor extension
                } else {
                    throw format_error("unknown line number opcode " + to_string((DW_LNE)opcode));
                }
        }
        if (cur->get_section_offset() > end) throw format_error("extended line number opcode exceeded its size");
