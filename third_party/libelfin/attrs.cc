...
#define AT_UDYNAMIC(name)                                                                             \
    uint64_t at_##name(const die &d, expr_context *ctx) { return _at_udynamic(DW_AT::name, d, ctx); } \
    static_assert(true, "")

static uint64_t _at_udynamic(DW_AT attr, const die &d, expr_context *ctx, int depth = 0) {
    // DWARF4 section 2.19
    if (depth > 16) throw format_error("reference depth exceeded for " + to_string(attr));
    ...
    switch (v.get_type()) {
        ...
        default:
            throw format_error(to_string(attr) + " has unexpected type " + to_string(v.get_type()));
    }
}
...
taddr at_high_pc(const die &d) {
    ...
    switch (v.get_type()) {
        ...
        default:
            throw format_error(to_string(DW_AT::high_pc) + " has unexpected type " + to_string(v.get_type()));
    }
}
...
expr_result at_data_member_location(const die &d, expr_context *ctx, taddr base, taddr pc) {
    ...
    switch (v.get_type()) {
        ...
        default:
            throw format_error("DW_AT_data_member_location has unexpected type " + to_string(v.get_type()));
    }
}
