...
static inline constexpr shf operator&(shf a, shf b) { return (shf)((std::uint64_t)a & (std::uint64_t)b); }

static inline constexpr shf operator|(shf a, shf b) { return (shf)((std::uint64_t)a | (std::uint64_t)b); }

static inline constexpr shf operator^(shf a, shf b) { return (shf)((std::uint64_t)a ^ (std::uint64_t)b); }

static inline constexpr shf operator~(shf a) { return (shf) ~((std::uint64_t)a); }
...
static inline constexpr pf operator&(pf a, pf b) { return (pf)((std::uint64_t)a & (std::uint64_t)b); }
...
struct Sym<Elf64, Order> {
    ...
    void set_binding(stb v) const { info = (info & 0xF) | ((unsigned char)v << 4); }
