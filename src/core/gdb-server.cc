...
struct PCSX::GdbServer::Private {
    uv_tcp_t server;
    GdbClient::ListType clients;
};
...
PCSX::GdbServer::~GdbServer() { delete m_priv; }

void PCSX::GdbServer::stopServer() {
    assert(m_serverStatus == SERVER_STARTED);
    m_serverStatus = SERVER_STOPPING;
    for (auto& client : m_priv->clients) client.close();
    uv_close(reinterpret_cast<uv_handle_t*>(&m_priv->server), closeCB);
}

void PCSX::GdbServer::startServer(uv_loop_t* loop, int port) {
    assert(m_serverStatus == SERVER_STOPPED);

    uv_tcp_init(loop, &m_priv->server);
    m_priv->server.data = this;
    ...
    result = uv_tcp_bind(&m_priv->server, reinterpret_cast<const sockaddr*>(&bindAddr), 0);
    if (result != 0) {
        uv_close(reinterpret_cast<uv_handle_t*>(&m_priv->server), closeCB);
        return;
    }
    result = uv_listen((uv_stream_t*)&m_priv->server, 16, onNewConnectionTrampoline);
    if (result != 0) {
        uv_close(reinterpret_cast<uv_handle_t*>(&m_priv->server), closeCB);
        return;
    }
...
void PCSX::GdbServer::onNewConnection(int status) {
    if (status < 0) return;
    GdbClient* client = new GdbClient(&m_priv->server);
    if (client->accept(&m_priv->server)) {
        m_priv->clients.push_back(client);
    } else {
        delete client;
    }
}
