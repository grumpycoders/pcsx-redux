    ...
    switch (value) {
        ...
        case 0x81:  // start memcard
            ...
            if (PCSX::g_emulator.config().NoMemcard ||
                ...
                if (PCSX::g_emulator.settings.get<PCSX::Emulator::SettingMcd2>().value.string().empty())
                ...
                if (s_rdwr == 2) {
                    switch (s_ctrlReg & 0x2002) {
                        case 0x0002:
                            ...
                            SaveMcd(PCSX::g_emulator.settings.get<PCSX::Emulator::SettingMcd1>().value.string().c_str(), g_mcd1Data,
                                    (s_adrL | (s_adrH << 8)) * 128,
                                    128);
                            break;
                        case 0x2002:
                            memcpy(g_mcd2Data + (s_adrL | (s_adrH << 8)) * 128, &s_buf[1], 128);
                            SaveMcd(PCSX::g_emulator.settings.get<PCSX::Emulator::SettingMcd2>().value.string().c_str(), g_mcd2Data,
                                    (s_adrL | (s_adrH << 8)) * 128,
                                    128);
                            break;
                    }
