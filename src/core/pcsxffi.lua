...
Breakpoint* addBreakpoint(uint32_t address, enum BreakpointType type, unsigned width, const char* cause, bool (*invoker)(uint32_t address, unsigned width));
...
local function defaultInvoker(address, width)
    C.pauseEmulator()
    return true
end
...
local function addBreakpoint(address, bptype, width, cause, invoker)
    ...
    if invoker ~= nil then
        ...
        invokercb = function(address, width)
            local ret = invoker(address, width)
            if ret == false then
                return false
            else
                return true
    ...
    local invokercb = ffi.cast('bool (*)(uint32_t address, unsigned width)', invokercb)
