...
class CDRiso {
  ...
  private:
    ...
    typedef ssize_t (CDRiso::*read_func_t)(std::unique_ptr<File>& f, unsigned int base, void* dest, int sector);

    int64_t m_cdOpenCaseTime = 0;
    bool m_useCompressed = false;

    std::unique_ptr<File> m_cdHandle;
    std::unique_ptr<File> m_subHandle;
    ...
    std::unique_ptr<File> m_decoded_ecm = NULL;
    ...
    struct trackinfo {
        ...
        std::unique_ptr<File> handle = nullptr;                            // for multi-track images CDDA
    ...
    ssize_t cdread_normal(std::unique_ptr<File>& f, unsigned int base, void* dest, int sector);
    ssize_t cdread_sub_mixed(std::unique_ptr<File>& f, unsigned int base, void* dest, int sector);
    ssize_t cdread_compressed(std::unique_ptr<File>& f, unsigned int base, void* dest, int sector);
    ssize_t cdread_2048(std::unique_ptr<File>& f, unsigned int base, void* dest, int sector);
    ssize_t cdread_ecm_decode(std::unique_ptr<File>& f, unsigned int base, void* dest, int sector);
    int handleecm(const char* isoname, std::unique_ptr<File>& cdh, int32_t* accurate_length);
    void PrintTracks();
    int aropen(std::unique_ptr<File>& fparchive, const char* _fn);
    int cdread_archive(std::unique_ptr<File>& f, unsigned int base, void* dest, int sector);
