...
struct trackinfo {
    ...
    enum { NONE = 0, BIN = 1, CCDDA = 2 } cddatype;  // BIN, WAV, MP3, APE
    void *decoded_buffer;
...
unsigned int msf2sec(char *msf) { return ((msf[0] * 60 + msf[1]) * 75) + msf[2]; }
...
static int parsetoc(const char *isofile) {
        ...
        if (!strcmp(token, "TRACK")) {
            ...
            if (!strncmp(token, "MODE2_RAW", 9)) {
                ...
                if (token != NULL && !strncmp(token, "RW", 2)) {
                    ...
                    subChanMixed = TRUE;
                    if (!strncmp(token, "RW_RAW", 6)) subChanRaw = TRUE;
                }
    ...
    if (numtracks > 0) cdHandle = fopen(filename, "rb");
    ...
    while (fgets(linebuf, sizeof(linebuf), fi) != NULL) {
        ...
        if (!strcmp(token, "TRACK")) {
            ...
            if (strstr(linebuf, "AUDIO") != NULL) {
                ...
                if (handleecm(filepath, cdHandle, &accurate_len) == 0 || handlearchive(filepath, &accurate_len) == 0) {
            ...
            if (sscanf(linebuf, " INDEX %02d %8s", &t, time) != 2) SysPrintf(".cue: failed to parse INDEX\n");
            ...
            if (numtracks > 1 && pregapOffset == -1) pregapOffset = ti[numtracks].start_offset / sector_size;
...
static int parsemds(const char *isofile) {
    ...
    for (i = 1; i <= numtracks; i++) {
        ...
        fread(&l, 1, sizeof(unsigned int), fi);
        l = SWAP32(l);
        if (l != 0 && i > 1) pregapOffset = msf2sec(ti[i].start);
...
static int handlepbp(const char *isofile) {
    ...
    if (strcmp(psar_sig, "PSTITLEIMG") == 0) {
        ...
        if (cdrIsoMultidiskSelect >= cdrIsoMultidiskCount) cdrIsoMultidiskSelect = 0;
...
static int handlecbin(const char *isofile) {
    ...
    if ((long long)index << ciso_hdr.align >= 0x80000000ll) SysPrintf("warning: ciso img too large, expect problems\n");
...
static int cdread_compressed(FILE *f, unsigned int base, void *dest, int sector) {
    ...
    if (fseek(cdHandle, start_byte, SEEK_SET) != 0) {
        SysPrintf("seek error for block %d at %x: ", block, start_byte);
        perror(NULL);
        return -1;
    }
    ...
    if (fread(is_compressed ? compr_img->buff_compressed : compr_img->buff_raw[0], 1, size, cdHandle) != size) {
...
static int cdread_ecm_decode(FILE *f, unsigned int base, void *dest, int sector) {
    ...
    boolean processsectors =
        (boolean)decoded_ecm_sectors;       // this flag tells if to decode all sectors or just skip to wanted sector
...
static int cdread_archive(FILE *f, unsigned int base, void *dest, int sector) {
    ...
    if (a != NULL /*&& (ecm_file_detected || sector*CD_FRAMESIZE_RAW <= len_uncompressed_buffer)*/) {
        ...
        for (fseek(cdimage_buffer, offset, SEEK_SET); offset < readsize;) {
            ...
            offset += size;
            SysPrintf("ReadArchive seek:%u(%u) cur:%u(%u)\r", sector, readsize / 1024, offset / CD_FRAMESIZE_RAW,
                      offset / 1024);
...
static unsigned char *CALLBACK ISOgetBuffer(void) { return cdbuffer + 12; }
...
static void PrintTracks(void) {
    ...
    for (i = 1; i <= numtracks; i++) {
        SysPrintf(_("Track %.2d (%s) - Start %.2d:%.2d:%.2d, Length %.2d:%.2d:%.2d\n"), i,
                  (ti[i].type == DATA ? "DATA" : ti[i].cddatype == CCDDA ? "CZDA" : "CDDA"), ti[i].start[0],
...
static long CALLBACK ISOreadTrack(unsigned char *time) {
    ...
    if (pregapOffset) {
        ...
        if (sector >= pregapOffset) {
            sector -= 2 * 75;
            if (sector < pregapOffset) subChanMissing = TRUE;
        }
    }
...
long CALLBACK ISOreadCDDA(unsigned char m, unsigned char s, unsigned char f, unsigned char *buffer) {
    ...
    for (track = numtracks;; track--) {
        ...
        if (track_start <= cddaCurPos) break;
        if (track == 1) break;
    }
    ...
    ret = cdimg_read_func(ti[file].handle, ti[track].start_offset, buffer, cddaCurPos - track_start);
...
int cdrIsoActive(void) { return (cdHandle != NULL || ecm_savetable != NULL || decoded_ecm != NULL); }
