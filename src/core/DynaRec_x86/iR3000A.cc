...
void DynaRecCPU::recLB() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0) {
            if (!_Rt_) return;
            gen.movsx(edi, Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]]);
            return;
        }
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (!_Rt_) return;
            gen.movsx(edi, Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]]);
            return;
        }
    }
...
void DynaRecCPU::recLBU() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0) {
            if (!_Rt_) return;
            gen.movzx(edi, Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]]);
            return;
        }
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (!_Rt_) return;
            gen.movzx(edi, Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]]);
            return;
        }
    }
...
void DynaRecCPU::recLH() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0) {
            if (!_Rt_) return;
            gen.movsx(edi, word[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]]);
            return;
        }
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (!_Rt_) return;
            gen.movsx(edi, word[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]]);
            return;
        }
    }
...
void DynaRecCPU::recLHU() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0) {
            if (!_Rt_) return;
            gen.movzx(edi, word[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]]);
            return;
        }
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (!_Rt_) return;
            gen.movzx(edi, word[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]]);
            return;
        }
        if (t == 0x1f80) {
...
void DynaRecCPU::recLW() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0) {
            if (!_Rt_) return;
            gen.mov(edi, dword[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]]);
            return;
        }
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (!_Rt_) return;
            gen.mov(edi, dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]]);
            return;
        }
        if (t == 0x1f80) {
            switch (addr) {
                ...
                case 0x1f8010f4:
                    if (!_Rt_) return;
                    gen.mov(edi, dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xffff]]);
                    return;
...
void DynaRecCPU::recLWL() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0) {
            iLWLk(addr & 3, (uint32_t)&PCSX::g_emulator->m_mem->m_wram[addr & 0x1ffffc]);
            return;
        }
        if (t == 0x1f80 && addr < 0x1f801000) {
            iLWLk(addr & 3, (uint32_t)&PCSX::g_emulator->m_mem->m_hard[addr & 0xffc]);
            return;
        }
        gen.mov(eax, m_iRegs[_Rs_].k + _Imm_);
    } else {
...
void DynaRecCPU::recLWR() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0) {
            iLWRk(addr & 3, (uint32_t)&PCSX::g_emulator->m_mem->m_wram[addr & 0x1ffffc]);
            return;
        }
        if (t == 0x1f80 && addr < 0x1f801000) {
            iLWRk(addr & 3, (uint32_t)&PCSX::g_emulator->m_mem->m_hard[addr & 0xffc]);
            return;
        }
        gen.mov(eax, m_iRegs[_Rs_].k + _Imm_);
    } else {
...
void DynaRecCPU::recSB() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0 && (t & 0x1fff) != 0) {
            if (IsConst(_Rt_)) {
                gen.mov(Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]], (uint8_t)m_iRegs[_Rt_].k);
            } else {
                gen.mov(al, Xbyak::util::byte[&m_regs.GPR.r[_Rt_]]);
                gen.mov(Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]], al);
            }
        ...
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (IsConst(_Rt_)) {
                gen.mov(Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]], (uint8_t)m_iRegs[_Rt_].k);
            } else {
                gen.mov(al, Xbyak::util::byte[&m_regs.GPR.r[_Rt_]]);
                gen.mov(Xbyak::util::byte[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]], al);
            }
            return;
        }
...
void DynaRecCPU::recSH() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0 && (t & 0x1fff) != 0) {
            if (IsConst(_Rt_)) {
                gen.mov(word[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]], (uint16_t)m_iRegs[_Rt_].k);
            } else {
                gen.mov(ax, word[&m_regs.GPR.r[_Rt_]]);
                gen.mov(word[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]], ax);
            }
        ...
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (IsConst(_Rt_)) {
                gen.mov(word[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]], (uint16_t)m_iRegs[_Rt_].k);
            } else {
                gen.mov(ax, word[&m_regs.GPR.r[_Rt_]]);
                gen.mov(word[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]], ax);
            }
            return;
        }
        if (t == 0x1f80) {
...
void DynaRecCPU::recSW() {
    ...
    if (IsConst(_Rs_)) {
        ...
        if ((t & 0x1fe0) == 0 && (t & 0x1fff) != 0) {
            if (IsConst(_Rt_)) {
                gen.mov(dword[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]], m_iRegs[_Rt_].k);
            } else {
                gen.mov(eax, dword[&m_regs.GPR.r[_Rt_]]);
                gen.mov(dword[&PCSX::g_emulator->m_mem->m_wram[addr & 0x1fffff]], eax);
            }
        ...
        if (t == 0x1f80 && addr < 0x1f801000) {
            if (IsConst(_Rt_)) {
                gen.mov(dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]], m_iRegs[_Rt_].k);
            } else {
                gen.mov(eax, dword[&m_regs.GPR.r[_Rt_]]);
                gen.mov(dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xfff]], eax);
            }
            return;
        }
        if (t == 0x1f80) {
            switch (addr) {
                ...
                case 0x1f8010f0:
                    if (IsConst(_Rt_)) {
                        gen.mov(dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xffff]], m_iRegs[_Rt_].k);
                    } else {
                        gen.mov(eax, dword[&m_regs.GPR.r[_Rt_]]);
                        gen.mov(dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xffff]], eax);
                    }
                    return;
...
#if 0
        if ((t & 0x1fe0) == 0 && (t & 0x1fff) != 0) {
            gen.MOV32MtoR(eax, (uint32_t)&PCSX::g_emulator->m_mem->m_wram[addr & 0x1ffffc]);
            iSWLk(addr & 3);
            gen.MOV32RtoM((uint32_t)&PCSX::g_emulator->m_mem->m_wram[addr & 0x1ffffc], eax);
            return;
        }
#endif
        if (t == 0x1f80 && addr < 0x1f801000) {
            gen.mov(eax, dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xffc]]);
            iSWLk(addr & 3);
            gen.mov(dword[&PCSX::g_emulator->m_mem->m_hard[addr & 0xffc]], eax);
            return;
        }
    }
