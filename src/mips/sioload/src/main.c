...
typedef struct { uint32_t inited, mode, ctrl, baud; } port_config_t;
...
int sio_put_byte(uint8_t data, uint32_t timeout)
{
	...
	if(timeout == 0) while(!(sio_get_status() & SIO_STAT_TX_RDY));
	else
	{
        uint32_t tries = 0;
...
void sioload()
{
    ...
    while(1)
        sio_put_byte('X',0); // sends an X to pc
...
int DelSIO(void)
{
    ...
    if (open("tty00:", O_RDONLY) != stdin)
        return 1;
    if (open("tty00:", O_WRONLY) != stdout)
        return 1;
    return 0;
}
