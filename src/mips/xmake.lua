...
local function _bin2c_sanitize_symbol(prefix, input)
    ...
    if symbol:match('^[0-9]') then symbol = '_' .. symbol end
...
local function _bin2c_sanitize_basename(input)
    ...
    if name:match('^[0-9]') then name = '_' .. name end
    return name
end

local function _bin2c_read(io, pathname)
    local f = io.open(pathname, 'rb')
    if not f then raise('bin2c: unable to read input file \'%s\'', pathname) end
...
local function _bin2c_write(io, os, pathname, data)
    ...
    if not f then raise('bin2c: unable to write output file \'%s\'', pathname) end
    f:write(data)
    f:close()
end
...
local function _bin2c_generate(io, os, input, output_c, output_h, symbol, compat_name)
    ...
    local bytes = {}
    for i = 1, size do bytes[#bytes + 1] = string.format('0x%02x', blob:byte(i)) end

    local lines = {}
    if size > 0 then
        for i = 1, #bytes, 12 do
            local chunk = {}
            for j = i, math.min(i + 11, #bytes) do chunk[#chunk + 1] = bytes[j] end
    ...
    local cfile = table.concat({
        string.format('#include "%s"', header_name), '', string.format('const unsigned char %s[] = {', symbol),
        ...
        string.format('__asm__(".globl _binary_%s_end\\n_binary_%s_end = %s + %d");', compat_name, compat_name, symbol,
                      size), string.format('const unsigned int _binary_%s_size = %du;', compat_name, size), '',
    }, '\n')

    local hfile = table.concat({
        '#pragma once', '#include <stddef.h>', '', '#ifdef __cplusplus', 'extern "C" {', '#endif', '',
        string.format('extern const unsigned char %s[];', symbol), string.format('extern const size_t %s_len;', symbol),
        ...
        string.format('extern const unsigned int _binary_%s_size;', compat_name), '', '#ifdef __cplusplus', '}',
        '#endif', '',
    }, '\n')
...
local function _bin2c_collect_inputs(target)
    ...
    for _, entry in ipairs(entries) do
        local pattern = entry
        if not path.is_absolute(pattern) then pattern = path.join(target:scriptdir(), pattern) end

        local matches = os.files(pattern)
        if #matches == 0 and os.isfile(pattern) then matches = { pattern } end

        if #matches == 0 then raise('nugget.bin2c: input \'%s\' did not match any file', entry) end
...
rule('nugget.bin2c', function()
    before_config(function(target)
        ...
        if output_dir then
            if not path.is_absolute(output_dir) then output_dir = path.join(target:scriptdir(), output_dir) end
        else
...
rule('ps-exe', function()
    ...
    before_config(function(target)
        ...
        if not extension or extension == '.psexe' then target:set('extension', '.ps-exe') end
        if target:get('kind') ~= 'binary' then raise('nugget: kind must be \'binary\' for ps-exe rule') end
    end)
end)
