...
CESTER_TEST(nclip_ccw, gte_tests, cop2_put(12, 0x00000000);  // (0,0)
            cop2_put(13, 0x00000064);                        // (100,0)
            cop2_put(14, 0x00640000);                        // (0,100)
            gte_clear_flag(); cop2_cmd(COP2_NCLIP); int32_t mac0; cop2_get(24, mac0); cester_assert_int_eq(10000, mac0);
            cester_assert_uint_eq(0, gte_read_flag());)

CESTER_TEST(nclip_cw, gte_tests, cop2_put(12, 0x00000000); cop2_put(13, 0x00640000);  // (0,100)
            cop2_put(14, 0x00000064);                                                 // (100,0)
            gte_clear_flag(); cop2_cmd(COP2_NCLIP); int32_t mac0; cop2_get(24, mac0);
            cester_assert_int_eq(-10000, mac0);)

CESTER_TEST(nclip_collinear, gte_tests, cop2_put(12, 0x00000000); cop2_put(13, 0x00320032);  // (50,50)
            cop2_put(14, 0x00640064);                                                        // (100,100)
            gte_clear_flag(); cop2_cmd(COP2_NCLIP); int32_t mac0; cop2_get(24, mac0); cester_assert_int_eq(0, mac0);)
...
CESTER_TEST(nclip_overflow, gte_tests,
            ...
            gte_clear_flag(); cop2_cmd(COP2_NCLIP); int32_t mac0; uint32_t flag; cop2_get(24, mac0);
            flag = gte_read_flag(); ramsyscall_printf("NCLIP overflow: MAC0=%d FLAG=0x%08x\n", mac0, flag);
            // Check if FLAG.16 or FLAG.15 (MAC0 overflow) is set
            ramsyscall_printf("  FLAG.16=%u FLAG.15=%u\n", (flag >> 16) & 1, (flag >> 15) & 1);
            cester_assert_int_eq(131071, mac0); uint32_t f15 = (flag >> 15) & 1; cester_assert_uint_eq(1, f15);)
