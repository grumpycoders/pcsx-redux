...
CESTER_TEST(sqr_shifted, gte_tests,
    GTE_WRITE_DATA(9, 0x1000);   // 1.0
    GTE_WRITE_DATA(10, 0x0800);  // 0.5
    GTE_WRITE_DATA(11, 0x2000);  // 2.0
    gte_clear_flag();
...
CESTER_TEST(sqr_saturation_shifted, gte_tests,
    GTE_WRITE_DATA(9, 0x4000);  // 4.0 in 4.12; 4^2 = 16, >>12 = 0x4000 (fits)
    GTE_WRITE_DATA(10, 0x5a82); // ~5.656 (sqrt(32)); 32 >>12 = 0x8000 = saturates
    GTE_WRITE_DATA(11, 0x7fff); // max positive; 0x7fff^2 >>12 = huge, saturates
    gte_clear_flag();
    GTE_EXEC(GTE_CMD_SQR_SF);
    uint32_t ir1, ir2, ir3;
    uint32_t flag;
...
CESTER_TEST(sqr_negative_input, gte_tests,
    GTE_WRITE_DATA(9, 0xfffffff6);  // -10 (sign-extended)
    GTE_WRITE_DATA(10, 0xffffffce); // -50
    GTE_WRITE_DATA(11, 0xffffff9c); // -100
    gte_clear_flag();
    GTE_EXEC(GTE_CMD_SQR);
    int32_t mac1, mac2, mac3;
    GTE_READ_DATA(25, mac1);
