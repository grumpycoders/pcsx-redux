#!/usr/bin/env python3
"""Emit mdec-in.bin, the job file the MDEC roundtrip rig reads over pcdrv.

The job used to be baked into the executable as a generated job.h. That started
as a workaround for PCopen returning -1 on the farm while PCcreat succeeded, and
THAT DIAGNOSIS WAS WRONG: PCDRV is complete and the protocol is fine; the
runner-agent had no references to input_fileset, so assets attached with --asset
were resolved, digest-pinned, shipped and then dropped. Fixed in the farm's
f9d1c4f, and the read half has since been probed from a console.

So the bake is gone and the guest binary no longer changes per arm: one build,
one job file per arm, and selecting an arm stops dirtying the tree. The layout is
documented at the top of mdec-roundtrip.c and the guest validates it.
...
# Job file, not a header. These four must match mdec-roundtrip.c's own constants;
# the guest validates the magic and both sizes rather than trusting the file.
JOB_FILE = 'mdec-in.bin'
ARM_FIELD = 12
OUT_BYTES = 16 * 16 * 3
MAX_RL_WORDS = 4096
...
if len(arm.encode()) >= ARM_FIELD:
    sys.exit(f'genjob.py: arm name {arm!r} does not fit the {ARM_FIELD}-byte field')

blob = b'MDRT'
blob += struct.pack('<I', 1 if upload else 0)      # flags: bit0 = upload scale table
blob += struct.pack('<I', len(rl))                 # rlWords
blob += struct.pack('<I', OUT_BYTES)               # outBytes the guest must receive
blob += struct.pack('<I', reset)                   # resetMode, 0 = no reset
blob += arm.encode().ljust(ARM_FIELD, b'\0')       # names the run and the output file
blob += bytes([quant]) * 128                       # Y table then UV table
blob += struct.pack('<64h', *st)                   # MDEC(3) scale matrix
blob += struct.pack('<%dH' % len(rl), *rl)

# The guest refuses a job whose rlWords exceeds its buffer, so say it here too
# rather than let a too-large arm be discovered on a console.
if len(rl) > MAX_RL_WORDS:
    sys.exit(f'genjob.py: {len(rl)} rl words exceeds the guest buffer of {MAX_RL_WORDS}')

open(JOB_FILE, 'wb').write(blob)
print(f'{JOB_FILE}: arm {arm}, upload={upload}, reset_mode={reset}, '
      f'{len(rl)} rl words, scale[0]={st[0]}, {len(blob)} bytes')
