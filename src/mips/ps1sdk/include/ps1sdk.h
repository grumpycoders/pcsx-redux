...
#define M_SUSPEND_INTR(__state_ptr) { *(__state_ptr) = GetCOP0_STATUS(); SetCOP0_STATUS(*(__state_ptr) & 0xFFFFFFFE); *(__state_ptr) &= 1; }
#define M_RESUME_INTR(__state_ptr) { SetCOP0_STATUS((*(__state_ptr) & 1) | (GetCOP0_STATUS() & 0xFFFFFFFE)); }
